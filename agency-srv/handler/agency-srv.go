...
func AddAgency(ctx context.Context, req *agencysrv.AddAgencyRequest, rsp *agencysrv.AddAgencyResponse) error {
	...
	if _, err := db.GetDB().Exec("insert into agency_profile_table (" +
		...
		"icon, photos) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", req.Agency.AgencyID, req.Agency.Name,
		req.Agency.Tel, req.Agency.Rating, req.Agency.Comments, req.Agency.Order,
		strings.Join(req.Agency.Tags, ","), req.Agency.Address, req.Agency.AddressDetail,
		req.Agency.Icon, strings.Join(req.Agency.Photos, ",")); err != nil {
